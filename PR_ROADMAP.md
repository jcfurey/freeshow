...
- **Known minor item (flag in PR, not a blocker):** the `remote` companion login logo uses `new URL("…/freeshow.webp", import.meta.url)`; Vite 8 blanks `import.meta` in iife output. Cosmetic, only that one companion. @vassbo said it's his "companion asset strategy" call — left for him / fixable on request.
